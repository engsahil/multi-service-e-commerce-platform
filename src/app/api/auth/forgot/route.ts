...
export async function POST(request: Request) {
  ...
  if (user) {
    ...
    await sendEmail({ to: user.email, subject: "Reset your Servixa password", html: `<div style="font-family:Arial,sans-serif"><h2>Servixa</h2><p>Hello ${user.name},</p><p>Your password reset link is valid for one hour.</p><p><a href="${url}">Reset password</a></p><p>If you did not request this, ignore this email.</p></div>` }).catch(console.error);
